...
public class RepositorySpecificationTest : BaseSetUpTest
{
    ...
    {
        var data = await accountRepository.GetAsync(expression: _ => true, takeAll: true)
    ...
    public async Task Get_account_must_return_valid_data_when_given_valid_input()
    {
        ...
        var specification = new AccountSearchSpecification(id);
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(data.Values.First().Id == id);
        });
    }
    ...
    {
        var specification = new AccountSearchSpecification(from: DateTimeHelper.VNDateTime.AddDays(10), to: DateTimeHelper.VNDateTime);
        var data = await accountRepository.GetAsync(specification);
        Assert.Multiple(() =>
        {
            Assert.NotNull(data);
    ...
    {
        var spec = new AccountSearchSpecification(from: DateTimeHelper.VNDateTime.AddDays(-10), to: DateTimeHelper.VNDateTime, pageSize: 10);
    ...
    public async Task Get_account_must_return_empty_when_give_invalid_id()
    {
        ...
        var spec = new AccountSearchSpecification(guid: id);
        var data = await accountRepository.GetAsync(spec);
        Assert.Multiple(() =>
        {
            Assert.NotNull(data);
    ...
    {
        var id = Guid.Parse("b5e5a3c0-e9e9-4528-a362-4ca4aaf43bf0");
        var spec = new AccountByIdRepoSpecification(id);
        var data = await accountRepository.GetAsync(spec);
        Assert.Multiple(() =>
        {
            Assert.NotNull(data);
            Assert.IsNotEmpty(data.Values);
            Assert.NotNull(data.Values.First());
            Assert.That(data.Values.First().Id == id);
        });
    }
    ...
    {
        var pageSize = 3;
        var spec = new AccountSearchSpecification(pageSize: pageSize);
        var data = await accountRepository.GetAsync(spec);
        Assert.Multiple(() =>
        {
            Assert.NotNull(data);
